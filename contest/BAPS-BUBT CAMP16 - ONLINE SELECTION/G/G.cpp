...
void update(int idx, int st, int ed, int i, int j, int v)
{
        if(st == i && ed == j)
        {
                loop(i, 3)
                {
                        if( v & (1<<i) )
                        {
                                sum[idx].cnt[i] = sum[idx].len - sum[idx].cnt[i];
        ...
        loop(i, 3)
        {
                if(lazy[idx][i])
                {
                        ...
                        sum[l].cnt[i] = sum[l].len - sum[l].cnt[i];
                        sum[r].cnt[i] = sum[r].len - sum[r].cnt[i];
        ...
        {
                update(r, mid+1, ed, i, j, v);
        }
        else
        {
                update(l, st, mid, i, mid, v);
                update(r, mid+1, ed, mid+1, j, v);
        }

        loop(i, 3)
        {
                sum[idx].cnt[i] = sum[l].cnt[i] + sum[r].cnt[i];
        }
}
...
DATA query(int idx, int st, int ed, int i, int j)
{
        ...
        else
        {
                ...
                loop(i, 3) ret.cnt[i] = 0;


                DATA tmp = query(l, st, mid, i, mid);

                loop(i, 3) ret.cnt[i] = tmp.cnt[i];

                tmp = query(r, mid+1, ed, mid+1, j);
