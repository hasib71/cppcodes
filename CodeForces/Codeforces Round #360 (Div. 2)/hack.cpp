...
void solve()
{
        ...
        loop(i, N)
        {
                ...
                for(int j=0; j<SZ(primes) && primes[j] < root; j++)
                {
                        if( (num[i] % primes[j]) == 0)
                        {
                                cnt = 0;
                                ...
                                mylcm[ primes[j] ] = max(mylcm[ primes[j] ], cnt);
...
void init()
{
        ...
        int till = 1007;
        int root = sqrt(till) + 7;

        for(int i=3; i<root; i+=2)
        {
                if(isprime[i])
                {
                        for(int j=i*i; j<till; j+=2*i)
                        {
                                isprime[j] = false;
                        }
                }
