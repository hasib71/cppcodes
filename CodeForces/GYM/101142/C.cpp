...
void solve()
{
        ...
        loop(i, N)
        {
                ...
                pos = query(1, 0, N-1, val, N-1);

                //vdump(pos);

                if(pos != INF)
                {
                        ptr[pos] = max(ptr[pos], i);        
                }
                //vdump(MP(pos, i));

                //cerr<<"Now Updating: "<<endl;
...
int main ()
{
    ...
    loop(i, N)
    {
            sf("%d %d", &ratings[i].fr.fr, &ratings[i].fr.sc);
            ratings[i].sc = i;
    }
