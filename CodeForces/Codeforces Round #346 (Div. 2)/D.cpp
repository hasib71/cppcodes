...
bool cross(pall A, pall B)
{
        return (A.fr * B.sc - A.sc*B.fr) > 0;
}
...
int main ()
{
    ...
    loop(i, N)
    {
            sf("%lld %lld", &points[i].fr, &points[i].sc);
    }
    ...
    {
        if(cross(points[j]-points[i], points[k] - points[j]))
        {
                cnt++;
        }
    }
