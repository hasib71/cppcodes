...
pair<pall, int> points[MAXX];
int N;


bool check(pall A, pall B, pall C)
{
	ll bx = B.fr - A.fr;
	ll by = B.sc - A.sc;
...
int main ()
{
    ...
    loop(i, N)
    {
	    sf("%lld %lld", &points[i].fr.fr, &points[i].fr.sc);
	    points[i].sc = i + 1;
    }
    ...
    for(int i = 2; i<N; i++)
    {
	    if(check(points[0].fr, points[1].fr, points[i].fr))
	    {
		    pf("%d %d %d\n", points[0].sc, points[1].sc, points[i].sc);
		    break;
	    }
    }
