...
int queryAndUpdate(int idx, int st, int ed, int n)
{
        sum[idx]--;
        ...
        else
        {
                ...
                if(sum[rgtIdx] > n)
                {
                        return queryAndUpdate(rgtIdx, mid + 1, ed, n);
                }
                else
                {
                        return queryAndUpdate(lftIdx, st, mid, n - sum[rgtIdx]);
                }
        }
}
...
int solve()
{
        ...
        for(int i=N-1; i>0; i--)
        {
                queryAndUpdate(1, 1, N, ara[i]);
        }

        return queryAndUpdate(1, 1, N, 0);
}
