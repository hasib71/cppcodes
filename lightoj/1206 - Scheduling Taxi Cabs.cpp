...
Passenger passengers[MAXX];





int operator-(const paii& a, const paii& b)
{
...
int main ()
{
    ...
    while(kases--)
    {
            ...
            loop(i, N)
            {
                    sf("%d:%d %d %d %d %d", &t1, &t2, &a, &b, &c, &d);

                    passengers[i].time = t1*60 + t2;
                    passengers[i].start = MP(a, b);
                    passengers[i].end = MP(c, d);
            }
            ...
            loop(i, N)
            {
                    for(int j=i+1; j<N; j++)
                    {
                            ...
                            if(passengers[i].time + ( passengers[i].start - passengers[i].end ) + (passengers[j].start - passengers[i].end) + 1 <= passengers[j].time)
                            {
                                    kuhn.adj[i].pb(j);
                            }
                    }
