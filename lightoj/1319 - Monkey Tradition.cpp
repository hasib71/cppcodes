...
/*
Find X, for X % num[i] = rem[i].
Prerequisities: for i != j => gcd(num[i], num[j]) = 1.
*/

ll chineseRem(ll num[], ll rem[], int n)
{
        ll prod = 1, result = 0, pp, mdinv;
        for(int i=0; i<n; i++) prod *= num[i];
        for(int i=0; i<n; i++) {
                pp = prod/num[i]; mdinv = modInv(pp, num[i]);
                if(mdinv >= prod) mdinv %= prod;
                pp *= mdinv; if(pp >= prod) pp %= prod;
                pp *= rem[i]; if(pp >= prod) pp %= prod;
                result = (result + pp) % prod;
        }
        return result;
}
