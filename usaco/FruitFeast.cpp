...
int main ()
{
    ...
    while(fullptr < SZ(full))
    {

        while(halfptr >=0 && (full[fullptr] + half[halfptr]) > T )
        {
            halfptr--;
        }

        if(halfptr >= 0 /* && (full[fullptr] + half[halfptr]) <= T */ )
        {
            mxPossible = max(mxPossible, full[fullptr] + half[halfptr] );
        }
