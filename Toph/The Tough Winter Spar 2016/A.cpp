...
void rec(int u)
{
        ...
        loop(i, SZ(graph[u]))
        {
                ...
                for(int j=0; j<=whichLevel[u]; j++)
                {
                        A[u][j] += ( bigmod( B[u], A[v][j+1], phiValues[j] ) );

                        if( A[u][j] >= phiValues[j] )
                        {
                                A[u][j] %= phiValues[j];
                        }
                }
