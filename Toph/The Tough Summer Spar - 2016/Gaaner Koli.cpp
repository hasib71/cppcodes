...
bool rec(int state, int last)
{
        bool &ret = dp[state][last];

        if(vis[state][last] == cc)
        {
                return ret;
        }
        ...
        loop(i, N)
        {
                if( (state & (1<<i)) || (last > 0 && musics[i].firstChar != last) ) continue;

                loop(j, SZ(musics[i].chars))
                {
                        if( rec( state | (1<<i), musics[i].chars[j]) == 0 )
                        {
                               ret = 1;
                               goto hell;
                        }
