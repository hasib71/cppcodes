...
void rec2(int mask, int pos, int cnt, int mymask)
{
        ...
        {
                rec2(mask, pos+1, cnt, mymask);

                if(cnt < (K/2) && ((mask & (1<<pos) != 0) ))
                {
                        rec2(mask, pos+1, cnt + 1, mymask | (1<<pos));
                }
        }
}


void rec(int pos, int cnt, int mask)
{
        if(pos >= N)
        {
                if(cnt == K)
