...
inline int valid(int cnt, char ch)
{
        if(cnt == 0)
        {
                if(ch == '1')
...
int visited[MAXX][4][4], cc;


ll rec(int pos, int a, int b)
{
        if(pos == (N-1))
        {
                if(a == b && a == 0)
        ...
        */
        ...
        int tmpA = valid(a, A[pos]);
        int tmpB = valid(b, B[pos]);

        if(tmpA >=0 && tmpB >= 0) 
            ret = rec( pos + 1, tmpA, tmpB );
