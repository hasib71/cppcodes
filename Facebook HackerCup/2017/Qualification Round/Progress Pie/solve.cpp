...
void solve()
{
        int bsq = SQ( (50 - X) ) + SQ( (50-Y) );

        int ret;

        if(bsq <= (50*50) )
        {
                int csq = SQ( (50 - X) ) + SQ ( (100 - Y) );

                double up = 50*50 + bsq - csq;
                double down = 100.0*sqrt(bsq);

                double angel = acos(up/down);

                angel = (180.0/PI) * angel;
