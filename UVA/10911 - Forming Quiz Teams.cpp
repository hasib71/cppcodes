...
{
    return sqrt(SQ(pos[i].fr - pos[j].fr) + SQ(pos[i].sc - pos[j].sc));
}
...
double rec(int state)
{
    ...
    if(visited[state] == kaseno)
    {
        return ret;
    }

    visited[state] = kaseno;
    ...
    loop(i, N)
    {
        ...
        for(int j=i+1; j<N; j++)
        {
            ...
            ret = min(ret, dist(i, j) + rec(state | (1<<i) | (1<<j)) );
...
int main ()
{
    ...
    while(true)
    {
        ...
        loop(i, N)
        {
            sf("%s %lf %lf", str, &pos[i].fr, &pos[i].sc);
        }
