...
int nodeIdInBaseArray[MAXX];

/// QTREE2

int chainPos[MAXX], chainSize[MAXX];
...
int getUp(int u, int k)
{
        ...
        int chnsz = chainPos[u];

        if(chnsz >= k)
        {
                return nodeIdInBaseArray[ posInBaseArray[u] - k ];
        }
        else
        {
...
int query_up(int u, int v)
{
        ...
        return ret + query_up(  parent[ chainHead[ whichChain[u] ] ]  , v );
}
